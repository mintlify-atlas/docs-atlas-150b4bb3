...
description: Protect OpenAI Codex CLI tool calls using Rampart's LD_PRELOAD syscall interception.
---
...
<Steps>
  <Step title="Start Rampart server">
    ```bash
    # Terminal 1
    rampart serve
    ```
  ...
  <Step title="Watch live decisions">
    ```bash
