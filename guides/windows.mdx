---
title: Windows Setup
description: Windows installation, configuration, and platform-specific limitations.
---
...
<Warning>
  On Windows, `rampart serve` runs in the foreground. Keep the terminal window open, or use Task Scheduler/NSSM to run it at startup.
</Warning>
...
<Steps>
  ...
  <Step title="Restore Rampart">
    Find Rampart, select "Restore" and "Allow on device"
  </Step>
</Steps>
...
<Steps>
  <Step title="Verify hooks are installed">
    ```powershell
    rampart doctor
    ```
  </Step>
  <Step title="Check settings file exists">
    ```powershell
    Test-Path ~\.claude\settings.json
    ```
  </Step>
  <Step title="Re-run setup if needed">
    ```powershell
...
<Steps>
  <Step title="Make sure rampart serve is running">
    ```powershell
    rampart serve
    ```
  </Step>
  <Step title="Check the serve URL matches">
    Default: `http://localhost:9090`
  </Step>
  <Step title="Test directly">
    ```powershell
    rampart test "your-command"
    ```
  </Step>
</Steps>
...
- [Native Ask Prompt](/guides/native-ask) — inline approval dialogs for sensitive commands
